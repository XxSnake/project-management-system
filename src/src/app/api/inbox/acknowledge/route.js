...
export async function POST(request) {
    try {
        ...
        const acknowledgedExceptions = addAcknowledgement(existing.acknowledgedExceptions, exceptionType);
        ...
        console.error('Acknowledge inbox exception error:', error);
        return NextResponse.json({ error: error.message || '标记已知忽略失败' }, { status: 500 });
    }
}
