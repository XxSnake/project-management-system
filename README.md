...
这是一个面向工程建设第三方检测业务的开源内部管理系统，用来把工作日志、合同、项目、人员和产值统计放到同一个流程里管理。
...
- Current stage: early-stage, self-hosted, internal single-user workflow first
...
这个项目主要服务于工程建设第三方检测机构的内部业务数字化，尤其适合以下场景：

- 从 WPS/Excel 工作日志中整理项目、人员、数量和工作内容。
- 从合同文件中提取价格清单并用于后续产值核算。
- 对无合同项目、疑似重复项目、异常工作记录进行集中审查。
- 按人员、项目、日期统计产值并导出报表。
- 在本地或内网环境中自托管轻量化业务系统。
...
- 增加登录鉴权和角色权限控制。
- 改进合同解析、价格清单识别和人工复核流程。
- 补充匿名演示数据、截图和更完整的部署说明。
- 增加工作日志导入、合同匹配、产值计算和报表导出的自动化测试。
- 增加 GitHub Actions，用于 Pull Request 的 lint、build 和基础检查。

这些任务会通过 GitHub Issues 跟踪，便于外部贡献者理解项目方向。
